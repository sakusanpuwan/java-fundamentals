...
- Access control modifiers - define visibility of classes, methods, constructors and fields
  ...
  | _(default)_     | Package-private | Classes in the **same package** only                     |
...
### Abstract vs Interface vs Class

| Situation                                                | Common Flow | Common Interface | Example                           | What to Use          |
| -------------------------------------------------------- | ----------- | ---------------- | --------------------------------- | -------------------- |
| All subclasses follow same overall process               | ✅ Yes      | ✅ Yes           | API callers, File Processing      | **Abstract class**   |
| All subclasses just share a method name (no shared flow) | ❌ No       | ✅ Yes           | Tasks with execute()              | **Interface**        |
| Classes have completely different responsibilities       | ❌ No       | ❌ No            | CustomerCreator, ReportDownloader | **Separate classes** |

**Abstract Class**

- Use when you have a common base flow/implementation that subclasses can follow but differ in some specific steps.
- Can have fields to store state and concrete methods with shared logic.

**Interface**

- Use when you just need to define a contract (method signatures) without any implementation so subclasses perform same task but differ in how they do it.
- Allows multiple inheritance as a class can implement multiple interfaces.

**Separate Classes**

- Use when classes have completely different responsibilities and no shared behavior or contract.
...
`Animal` is the reference type (parent or super class)  
`Dog` and `Cat` are the object types (child or sub classes)

This creates a `Dog` object in memory but treats it as a generic `Animal` type when I refer to it. `Dog` provides its own actual implementation of the `sound()` method, overriding the generic version in `Animal`.

`@Override`  
Indicates that the method overrides a method in a superclass. If the method doesn't correctly override the superclass
...
Why `Animal myDog = new Dog();` and not `Dog myDog = new Dog();`?
Using the parent class (`Animal`) as the reference type allows for polymorphism.

If you had a list and wanted to call `sound()` on each animal, you'd need one list per animal type because they're different classes, but with polymorphism you can have a single list of `Animal` references:

```java
    List<Animal> animals = List.of(new Dog(), new Cat(), new Dog());
    for (Animal animal : animals) {
        animal.sound(); // Dynamic dispatch to the correct subclass method
    }
```
